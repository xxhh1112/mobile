...
namespace Bit.iOS.Core.Services
{
    public class DeviceActionService : IDeviceActionService
    {
        ...
        public void RateApp()
        {
            string uri = "itms-apps://itunes.apple.com/us/app/id1137397744?action=write-review";
        ...
        {
            if(Application.Current is IApp currentApp && !currentApp.Options.IosExtension)
            {
        ...
        {
            if (Application.Current is IApp app && app.Options != null && !app.Options.IosExtension)
            {
                return app.DisplayActionSheetAsync(title, cancel, destruction, buttons);
            }
            var vc = GetPresentedViewController();
            if (vc is null)
            {
